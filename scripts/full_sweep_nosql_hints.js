...
import { initMongo, initNoSQLSampleData, executeMongo, executeMongoForUser } from '../backend/db.js';
...
async function main() {
  ...
  const sampleUsers = await executeMongo(async (mongoDb) => {
    return mongoDb.collection('sample_users').find({}).toArray();
  });

  const failures = [];

  await executeMongoForUser(SWEEP_USER, async (mongoDb) => {
    const audit = mongoDb.collection('audit_users');
    await audit.deleteMany({});
    if (sampleUsers.length) {
      await audit.insertMany(sampleUsers.map(d => {
        const c = { ...d };
        delete c._id;
        return c;
      }));
